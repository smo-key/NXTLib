...
//
// NXTLib.BluetoothWrapper.Bluetooth.Msft.WindowsBluetoothRadio
// 
...
namespace NXTLib.BluetoothWrapper.Bluetooth.Msft
{
    sealed class WindowsBluetoothRadio : IBluetoothRadio //: IDisposable
    {
        ...
        internal WindowsBluetoothRadio(IntPtr handle)
        {
            ...
            ReadVersionInfo(handle, ref _lmpV, ref _lmpSubv, ref _hciV, ref _hciRev);
        }
        #endregion

        #region ReadVersionInfo
        void ReadVersionInfo(IntPtr handle,
            ref LmpVersion lmpV, ref int lmpSubv, ref HciVersion hciV, ref int hciRev)
        {
            // Windows 7 IOCTL
            ...
            if (!success) {
                ...
                var lmpFeatures = (LmpFeatures)BitConverter.ToInt64(buf, OffsetOf_lmpFeatures);
#if DEBUG
        ...
        public HardwareStatus HardwareStatus
        {
            get
            {
                ...
                int hresult = NativeMethods.BluetoothGetRadioInfo(handle, ref tmp);
                if (hresult != 0) {
                    //throw new System.ComponentModel.Win32Exception(hresult, "Error retrieving Radio information.");
                ...
                switch (value) {
                    case RadioMode.Discoverable:
                        if (Mode == RadioMode.PowerOff) {
                            okConno = NativeMethods.BluetoothEnableIncomingConnections(handle, true);
                        }
                        okDisco = NativeMethods.BluetoothEnableDiscovery(handle, true);
                        break;
                    case RadioMode.Connectable:
                        if (Mode == RadioMode.Discoverable) {
                            okDisco = NativeMethods.BluetoothEnableDiscovery(handle, false);
                        } else {
                            okConno = NativeMethods.BluetoothEnableIncomingConnections(handle, true);
                        }
                        break;
                    case RadioMode.PowerOff:
                        ...
                        okConno = NativeMethods.BluetoothEnableIncomingConnections(handle, false);
                        break;
                }
        ...
        [SuppressMessage("Microsoft.Usage", "CA1806:DoNotIgnoreMethodResults", MessageId = "NXTLib.BluetoothWrapper.BluetoothAddress.TryParse(System.String,NXTLib.BluetoothWrapper.BluetoothAddress@)")]
        private static string GetRadioRegKey(NXTLib.BluetoothWrapper.BluetoothAddress radioAddress)
        {
            ...
            try {
                ...
                while (NativeMethods.SetupDiEnumDeviceInfo(hDevInfo, idx++, ref data)) {
                    ...
                    NXTLib.BluetoothWrapper.BluetoothAddress.TryParse(instanceId.Substring(lastSlashIndex + 1, instanceId.Length - lastSlashIndex - 1), out address);
                    //NXTLib.BluetoothWrapper.BluetoothAddress address = NXTLib.BluetoothWrapper.BluetoothAddress.Parse(instanceId.Substring(lastSlashIndex + 1, instanceId.Length - lastSlashIndex - 1));
